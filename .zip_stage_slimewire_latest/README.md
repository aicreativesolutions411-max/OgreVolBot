...
Web profiles, saved logins, PFPs, X handles, watchlists, presets, referral settings, and web sessions are also stored under `DATA_DIR` in `web-auth.json`. Keep `DATA_DIR` on persistent Render storage for users to stay logged in through deploys.
