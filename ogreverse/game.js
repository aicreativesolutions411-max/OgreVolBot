(() => {
  ...
  // Base path the game is served from ("" at root, "/Ogreverse" when reverse-proxied under
  // slimewire.org/Ogreverse) so all API calls resolve correctly behind the proxy.
  const OGV_BASE = (typeof location !== "undefined" && location.pathname ? location.pathname.replace(/\/[^/]*$/, "") : "");
  function apiUrl(p) { return `${OGV_BASE}${p}`; }
