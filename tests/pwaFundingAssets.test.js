...
test("Cash and Fun source shells install the same funding release", () => {
  ...
  assert.match(publicCashHtml, /slimecash-build" content="24"/);
  assert.match(publicCashHtml, /cash\.css\?v=24/);
  assert.match(publicCashHtml, /cash\.js\?v=24/);
  assert.match(publicCashWorker, /cash\.css\?v=24/);
  assert.match(publicCashWorker, /cash\.js\?v=24/);
...
test("installed Cash and Fun apps force worker updates and isolate their caches", () => {
  ...
  assert.match(publicCashWorker, /const CACHE = "slimecash-v26"/);
