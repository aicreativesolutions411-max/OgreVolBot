...
test("Cash and Fun source shells install the same funding release", () => {
  ...
  assert.match(publicFunHtml, /fun\.css\?v=21/);
  assert.match(publicFunHtml, /fun\.js\?v=37/);
  assert.match(publicFunWorker, /fun\.css\?v=21/);
  assert.match(publicFunWorker, /fun\.js\?v=37/);
});
...
test("installed Cash and Fun apps force worker updates and isolate their caches", () => {
  ...
  assert.match(publicFunWorker, /const FUN_CACHE = "slimewire-fun-v26"/);
