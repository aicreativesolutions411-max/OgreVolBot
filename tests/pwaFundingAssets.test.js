...
test("Cash and Fun source shells install the same funding release", () => {
  ...
  assert.match(publicCashHtml, /slimecash-build" content="38"/);
  assert.match(publicCashHtml, /cash\.css\?v=38/);
  assert.match(publicCashHtml, /cash\.js\?v=38/);
  assert.match(publicCashWorker, /cash\.css\?v=38/);
  assert.match(publicCashWorker, /cash\.js\?v=38/);
  assert.match(publicFunHtml, /fun\.css\?v=66/);
  assert.match(publicFunHtml, /fun\.js\?v=89/);
  assert.doesNotMatch(publicFunHtml, /fun-indicators\.js\?v=7/);
  assert.match(publicFunJs, /loadFunScript\("\/fun-indicators\.js\?v=7"\)/);
  assert.match(publicFunWorker, /fun\.css\?v=66/);
  assert.match(publicFunWorker, /fun\.js\?v=89/);
...
test("installed Cash and Fun apps force worker updates and isolate their caches", () => {
  ...
  assert.match(publicCashWorker, /const CACHE = "slimecash-v40"/);
  assert.match(publicFunWorker, /"slimewire-fun-v86"/);
