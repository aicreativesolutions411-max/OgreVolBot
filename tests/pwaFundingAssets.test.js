...
test("Cash and Fun source shells install the same funding release", () => {
  ...
  assert.match(publicFunHtml, /fun\.css\?v=31/);
  assert.match(publicFunHtml, /fun\.js\?v=46/);
  assert.match(publicFunHtml, /fun-indicators\.js\?v=6/);
  assert.match(publicFunWorker, /fun\.css\?v=31/);
  assert.match(publicFunWorker, /fun\.js\?v=46/);
...
test("installed Cash and Fun apps force worker updates and isolate their caches", () => {
  ...
  assert.match(publicFunWorker, /const FUN_CACHE = "slimewire-fun-v39"/);
