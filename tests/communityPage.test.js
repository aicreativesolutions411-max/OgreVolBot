...
test("coin community has a pretty public route and responsive shell", () => {
  ...
  assert.ok(html.indexOf('/config.js') < html.indexOf('https://app.slimewire.org/community.js?v='), "runtime API config must load before the no-store community client");
  assert.match(html, /https:\/\/app\.slimewire\.org\/community\.js\?v=/);
