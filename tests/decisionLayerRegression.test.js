...
test("mobile Live Terminal, Live Pairs, and Slime Scope refresh without resetting scroll position", () => {
  assert.match(appSource, /MOBILE_STABLE_FEED_TABS = new Set\(\["terminal", "live", "slimeScope", "kol", "watchlist", "liveTrades", "sniper"\]\)/);
...
test("landing sponsor and KOL entries use the compact top ticker", () => {
  ...
  assert.doesNotMatch(indexSource, /share\.google/);
  assert.match(indexSource, /https:\/\/dexscreener\.com\/solana\/67fpweu4iar36zn9ttmfzoztpbruv2gepnrmjqbq32mc/);
  assert.match(indexSource, /aria-label="Open ASML on Dex"/);
  assert.match(indexSource, /https:\/\/dexscreener\.com\/solana\/5czf8ao1z4yrxzext2sk1b577cbd5j9naieybamuswy4/);
  assert.match(indexSource, /aria-label="Open USD2 on Dex"/);
  assert.match(indexSource, /https:\/\/x\.com\/USD2onSolana/);
  assert.match(indexSource, /https:\/\/usd2sol\.finance/);
  assert.match(indexSource, /aria-label="Open USD2 website"/);
  assert.match(indexSource, /https:\/\/t\.me\/USD2Solana/);
  assert.doesNotMatch(indexSource, /target="_blank" rel="noreferrer"/);
