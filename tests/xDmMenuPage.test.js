...
test("X DM uses short branded chart and Trade Pad redirects", () => {
  ...
  assert.match(menuUrl, /xDmPortalOrigin\(\)/);
  assert.match(chartUrl, /xDmPortalOrigin\(\)/);
  assert.doesNotMatch(menuUrl, /xDmRedirectOrigin\(\)/);
  assert.doesNotMatch(chartUrl, /xDmRedirectOrigin\(\)/);
  ...
  assert.match(server, /OPEN SLIMEWIRE CHART \+ TRADE PAD/);
  assert.match(functionBody(server, "xDmSlotMenuText"), /const primaryUrl = linked \? xDmMenuUrl/);
