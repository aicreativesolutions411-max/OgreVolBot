...
test("Wallet Launch Snipe is launch-only and supports Solana creators plus Robinhood deployers", () => {
  ...
  assert.match(functionBody(serverSource, "executeRhWalletLaunchSnipe"), /webRhBundle\(/);
  assert.match(functionBody(serverSource, "executeRhWalletLaunchSnipe"), /tradeAttemptId: `wallet-launch-rh-\$\{plan\.id\}-\$\{tokenAddress\.toLowerCase\(\)\}`/);
  ...
  assert.match(functionBody(serverSource, "processWalletLaunchWatchPlan"), /plan\.seenLaunches = uniqueStrings[\s\S]*WALLET_LAUNCH_SEEN_LIMIT[\s\S]*executeRhWalletLaunchSnipe[\s\S]*executeSolWalletLaunchSnipe/);
  assert.match(serverSource, /const WALLET_LAUNCH_SEEN_LIMIT = 5_000/);
  assert.match(functionBody(serverSource, "webCreateWalletLaunchSnipe"), /durableTradeWatchKey\(\[\s*userId, "wallet_launch_snipe", chain, launchWallet\s*\]\)/);
  ...
  assert.match(functionBody(indexSource, "renderWalletLaunch"), /Existing coins and the wallet&apos;s normal trades are ignored/);
  assert.match(functionBody(indexSource, "renderWalletLaunch"), /walletPublicKeys/);
  assert.match(functionBody(indexSource, "renderWalletLaunch"), /walletLaunchTargetMode==="single"[\s\S]*walletLaunchTargetMode==="bundle"/);
  assert.match(functionBody(indexSource, "walletLaunchWatchListHtml"), /Bundle ·/);
  assert.match(functionBody(indexSource, "walletLaunchWatchListHtml"), /Single wallet/);
  assert.match(functionBody(indexSource, "walletLaunchWatchListHtml"), /Stop &amp; remove/);
...
test("group and channel settings can be managed privately in DM without setup chatter", () => {
  ...
  assert.match(command, /privateSettings\?\.\[1\] \|\| ""/);
  assert.match(command, /showGroupBotDmRoomList\(chat\.id, userId, null,/);
  const rooms = functionBody(serverSource, "groupBotDmAdminRooms");
  assert.match(rooms, /isGroupBotAdmin\(room\.chatId, userId\)/);
  assert.match(rooms, /runWithConcurrency\(candidates, 5/);
  assert.match(rooms, /telegram\("getChat"/);
  const matchRoom = functionBody(serverSource, "groupBotDmRoomMatches");
  assert.match(matchRoom, /entry\?\.token/);
  assert.match(matchRoom, /username/);
  assert.match(matchRoom, /title\.includes\(bare\)/);
  const showRooms = functionBody(serverSource, "showGroupBotDmRoomList");
  assert.match(showRooms, /query && rooms\.length === 1/);
  assert.match(showRooms, /rememberGroupBotDmMenuTarget/);
