...
function normalizeImageUrl(value) {
  const text = String(value || "").trim();
  if (!text) return "";
  if (/^\/api\/web\/token-image\?/i.test(text)) return text;
  if (/^ipfs:\/\//i.test(text)) {
...
function tokenImageProxyUrl(row = {}) {
  ...
  return `
    <section class="trade-layout">
      <article class="trade-card slime-swap-card ogre-swap-card ogre-swap-skin">
        ...
        <p class="trade-status oss-status" data-trade-status>${state.tradeResult ? escapeHtml(state.tradeResult.message || "Trade complete.") : "Ready."}</p>
      </article>
