(function () {
  ...
  function activityTokenAmount(row = {}) {
    return row.tokenUiAmount !== null && row.tokenUiAmount !== undefined && row.tokenUiAmount !== "" ? row.tokenUiAmount : row.tokenAmount;
  }
  ...
  function activityRowHtml(row, metaByToken) {
    ...
    const tokenAmount = activityTokenAmount(row);
    const left = row.leftText || (side === "buy" ? `${compactAmount(row.solAmount ?? row.amountSol)} SOL` : `${row.sellPercent ? `${compactAmount(row.sellPercent)}% ` : compactAmount(tokenAmount) + " "}$${symbol}`);
    const right = row.rightText || (side === "buy" ? `${compactAmount(tokenAmount, "Token amount pending")} $${symbol}` : `${compactAmount(row.solAmount, "SOL pending")} SOL`);
    ...
    const signatureLabel = String(row.signatureLabel || "Transaction"), explorerLabel = row.chain === "robinhood" || isRobinhood(token) ? "Explorer ↗" : "Solscan ↗";
    ...
    return `<article class="proActivityRow">
      ...
      <div class="proActivityWallet"><b>${escapeHtml(wallet)}</b><small title="${escapeHtml(walletKey)}">${escapeHtml(short(walletKey) || "Wallet identity recorded")}</small><small>${escapeHtml(source)}</small>${signature ? `<small title="${escapeHtml(signature)}">${escapeHtml(signatureLabel)}: ${escapeHtml(short(signature))}</small>` : ""}</div>
      <div class="proActivityMeta"><b title="${escapeHtml(time.exact)}">${escapeHtml(time.relative)}</b><span>${row.targetMarketCapUsd ? `Target ${escapeHtml(formatUsd(row.targetMarketCapUsd))}` : escapeHtml(row.detail || "")}</span><div class="proActivityLinks">${token ? `<a href="${chartHash}">Chart</a>` : ""}${explorer ? `<a href="${escapeHtml(explorer)}" target="_blank" rel="noopener noreferrer">${explorerLabel}</a>` : ""}${row.canCancel ? `<button type="button" data-pro-cancel-global="${escapeHtml(row.id)}" data-chain="${escapeHtml(row.cancelChain || row.chain || "solana")}">Cancel</button>` : ""}</div></div>
    </article>`;
  }
  ...
  function normalizeOrderRows(orders, plans, rhGuards) {
    ...
    const planRows = (Array.isArray(plans) ? plans : []).flatMap((plan) => {
      ...
      return wallets.map((wallet, walletOffset) => {
        const status = String(wallet.exitStatus || wallet.status || plan.status || "watching"), needsAttention = ["outcome_unknown", "needs_attention"].includes(status.toLowerCase());
        const submissionSignature = wallet.buySubmissionSignature || wallet.submissionSignature || "", signature = wallet.sellSignature || submissionSignature || wallet.buySignature || "";
        const signatureLabel = wallet.sellSignature ? "Sell tx" : submissionSignature ? "Submission" : wallet.buySignature ? "Buy tx" : "Transaction";
        return { ...plan, ...wallet, id: `${plan.id || "plan"}:${wallet.publicKey || wallet.walletPublicKey || wallet.walletIndex || walletOffset}`, side: "sell", tokenMint: plan.tokenMint, walletLabel: wallet.label || wallet.walletLabel, walletPublicKey: wallet.publicKey || wallet.walletPublicKey, walletIndex: wallet.walletIndex, status, statusLabel: needsAttention ? "Needs attention" : status, needsAttention, isActive: !needsAttention && ["watching", "armed", "running", "pending_buy", "arming", "retrying", "submitting", "waiting_next_loop", "timer-only", "price-unavailable"].includes(status.toLowerCase()), sellPercent: wallet.triggerSellPercent || plan.triggerSellPercent || plan.sellPercent || 100, signature, signatureLabel, timestamp: wallet.soldAt || wallet.triggeredAt || wallet.buySubmissionSignedAt || wallet.submissionSignedAt || wallet.armedAt || wallet.updatedAt || wallet.lastCheckedAt || plan.completedAt || plan.updatedAt || plan.createdAt, sourceLabel: plan.source || "Automated exit", kindLabel: ["pending_buy", "arming"].includes(status.toLowerCase()) ? "Protected buy" : "Auto exit", detail: needsAttention ? "Outcome unknown — check the explorer before retrying." : [plan.takeProfitSummary, plan.stopLossSummary].filter(Boolean).join(" · "), dexUrl: plan.dexUrl };
      });
    });
  ...
  function normalizeRhActivity(rows) {
    return (Array.isArray(rows) ? rows : []).map((item) => {
      ...
      if (kind === "buy") {
        return { ...base, leftLabel: "Paid", leftText: item.amountSol ? `${compactAmount(item.amountSol)} SOL` : `${compactAmount(item.amountEth, "ETH pending")} ETH`, rightLabel: "Received", rightText: `${compactAmount(activityTokenAmount(item), "Token amount pending")} $${item.symbol || short(item.tokenAddress) || "TOKEN"}` };
      }
      if (kind === "sell" || kind === "auto-sell") {
        return { ...base, leftLabel: "Sold", leftText: `${compactAmount(activityTokenAmount(item), "Token amount pending")} $${item.symbol || short(item.tokenAddress) || "TOKEN"}`, rightLabel: "Received", rightText: item.amountSol ? `${compactAmount(item.amountSol)} SOL` : `${compactAmount(item.amountEth ?? item.out, "ETH pending")} ETH`, detail: item.trigger ? `${item.trigger}${item.movePct !== "" && item.movePct != null ? ` · ${item.movePct}%` : ""}` : "" };
      }
