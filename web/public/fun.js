...
(() => {
  ...
  function renderSocialProfile() {
    ...
    if (user.username) panel.insertAdjacentHTML("afterbegin", `<div class="read-card account-switch-card"><h3>Use another saved account</h3><p>Log in to restore that account's wallets, positions, presets, and profile on this device.</p><button class="submit-trade" type="button" data-fun-account="login">Log in / switch account</button></div>`);
    if (IS_WALLET_ROUTE) {
