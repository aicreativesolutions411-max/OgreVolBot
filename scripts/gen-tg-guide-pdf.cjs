...
// Render web/public/tg-guide.html to a print-ready PDF. Served over a tiny local HTTP server
// (NOT file://) so the absolute /assets/... paths resolve — that's what makes the cover logo,
// the hero background and the footer lockup render into the PDF instead of being dropped.
// Output lands in web/public/ so the page can offer a direct /SlimeWire-TG-Guide.pdf download.
...
(async () => {
  ...
  // Keep a copy under docs/ next to the user manual.
  ...
  await b.close();
  server.close();
  console.log("PDF written -> web/public/SlimeWire-TG-Guide.pdf (+ docs/ copy)");
