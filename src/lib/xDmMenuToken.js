...
export function signXDmMenuToken(secret, payload, options = {}) {
  ...
  const ttlMs = Math.max(60_000, Math.min(30 * 60_000, Number(options.ttlMs) || X_DM_MENU_DEFAULT_TTL_MS));
  const record = {
    kind: X_DM_MENU_KIND,
  ...
  if (!record.senderId || !record.userId || !record.mint) throw new Error("Missing X DM menu token scope");
...
export function verifyXDmMenuToken(secret, token, options = {}) {
  ...
  try {
    ...
    if (!record.senderId || !record.userId || !record.mint || !record.nonce) return null;
